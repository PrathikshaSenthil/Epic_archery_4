...
function preload(){
  backgroundImg = loadImage("assets/background3.gif")
}
...
function setup() {
  ...
  playerBase = new PlayerBase(300, random(450, height - 300), 180, 400);
  player = new Player(285, playerBase.body.position.y - 145, 100, 190);
...
function keyPressed() {
  ...
  if(keyCode === 32){
    ...
    var arrow = new PlayerArrow(posX, posY, 35, 30);
...
function handleComputerArcher() {
  if (!computerArcher.collapse && !playerArcher.collapse) {
    setTimeout(() => {
      ...
      var arrow = new ComputerArrow(pos.x, pos.y, 35, 30, angle);
